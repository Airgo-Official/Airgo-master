...
fun CouponScreen() {
    ...
    val list = sharedVM.storeVM.couponList.collectAsLazyPagingItems()
                    ...
                    item {
                        Column {
                            ...
                            Row(
                                ...
                                CouponTableColumn.entries.forEach { t ->
                                    Text(
                                            ...
                                            Row(
                                                ...
                                                SelectionContainer {
                                                    Text(
                                                        ...
                                                        modifier = Modifier.width(CouponTableColumn.ID.width)
                                                    )
                                                }
                                                SelectionContainer {
                                                    Text(
                                                        text = item.name,
                                                        modifier = Modifier.width(CouponTableColumn.NAME.width)
                                                    )
                                                }
                                                SelectionContainer {
                                                    Text(
                                                        text = Status.i18nByOrdinal(item.status),
                                                        modifier = Modifier.width(CouponTableColumn.STATUS.width)
                                                    )
                                                }
                                                SelectionContainer {
                                                    Text(
                                                        text = item.couponCode,
                                                        modifier = Modifier.width(CouponTableColumn.COUPON_CODE.width)
                                                    )
                                                }
                                                SelectionContainer {
                                                    Text(
                                                        text = CouponType.valueOf(item.couponType)
                                                            .i18n(),
                                                        modifier = Modifier.width(CouponTableColumn.COUPON_TYPE.width)
                                                    )
                                                }
                                                SelectionContainer {
                                                    Text(
                                                        text = item.discount.toString(),
                                                        modifier = Modifier.width(CouponTableColumn.DISCOUNT.width)
                                                    )
                                                }
                                                SelectionContainer {
                                                    Text(
                                                        text = item.minOrderAmount.toString(),
                                                        modifier = Modifier.width(CouponTableColumn.MIN_ORDER_AMOUNT.width)
                                                    )
                                                }
                                                SelectionContainer {
                                                    Text(
                                                        text = TimeUtils.toLocalDateString(item.createdAt),
                                                        modifier = Modifier.width(CouponTableColumn.CREATED_AT.width)
                                                    )
                                                }
                                                Row(
                                                    ...
                                                    IconButton(onClick = {
                                                        sharedVM.storeVM.initEditCoupon(
                                                            EditType.UPDATE,
                                                            item
                                                        )
                                                        navController.toEditCoupon()
                                                    ...
                                                    IconButton(onClick = {
                                                        sharedVM.dialogVM.openDialog(
                                                            ...
                                                            ACButtonError(onClick = {
                                                                scope.launch {
                                                                    Repository.remote.deleteCoupon(
                                                                        DeleteCouponReq(id = item.id)
                                                                    )
                                                                        .onFailure { r ->
                                                                            sharedVM.dialogVM.openDialog(
