...
@Composable
fun NodeScreen(list: List<NodeDestination> = NodeDestination.entries) {
    val pageState = rememberPagerState { list.size }
    Scaffold(
        topBar = { NodeScreenTopBar(pageState, list) }
    ) { paddingValues ->
        HorizontalPager(
            pageState,
...
@Composable
fun NodeScreenTopBar(pageState: PagerState, list: List<NodeDestination>): Unit {
